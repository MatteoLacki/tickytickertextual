...
- Folder-only mode defaults on across filesystem panes. A `.d` containing both `analysis.tdf` and `analysis.tdf_bin` previews as Description, the dataset-specific `MzAcqRangeLower`–`MzAcqRangeUpper`, gradient length from `MAX(Frames.Time) - MIN(Frames.Time)`, and human-readable file sizes instead of an internal listing. Cache this metadata. Show gradient length—but not acquisition m/z—in aligned current-pane rows. Sort ordinary folders, then `.d` folders, then files.
...
- `.d` directories can be collected in the interactive `:selected:` pane. Read `GlobalMetadata.Description`, `MzAcqRangeLower`, `MzAcqRangeUpper`, and the `Frames.Time` span through one read-only built-in `sqlite3` connection and reuse the cached object during selection and analysis. Acquisition bounds are display-only context. User-editable `mz_min`/`mz_max` are the primary comparison window passed to both fit and TIC APIs. Selected rows use aligned columns ordered as root-relative path, Description, below TIC, above TIC, and optional HeLa fit; omit acquisition m/z and gradient length there. One path may be marked as `:HELA CHOSEN:`, and removal must not touch the filesystem.
